...
std::map<DeviceId,MyVideoRender*> rendermap;
...
public:
		...
		if (result != 0)
		{
			...
			if (!g_videoOprate.empty()){
				if (g_videoOprate == "0"){ ///导入视频
					...
					fakeCamera.publishedQualities.setStreamOptions(rtc::StreamType::stream_main, cap, rtc::codec_h264);
	...
	virtual void onSubscribeResult(Result result, const DeviceId& fromId) {
		...
		if(result == AVD_OK){
			if(g_decodeable == "1" || g_decodeable == "2"){
				std::map<DeviceId,MyVideoRender*>::iterator it = rendermap.find(fromId);
				if(it == rendermap.end()){
	...
	virtual void onPublishLocalResult(Result result, const DeviceId& fromId) {
		...
		m_video->setVideoBitrate(fakeCamera.id, 4000 * 1000, 6000 * 1000);
	}
